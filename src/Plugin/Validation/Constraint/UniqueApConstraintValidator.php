...
class UniqueApConstraintValidator extends ConstraintValidator implements ContainerInjectionInterface {
  ...
  /**
   * Constructs a new UniqueApConstraintValidator instance.
   */
  ...
  public function validate($entity, Constraint $constraint) {
    ...
    // Query for existing APs with the same combination.
    $query = $this->entityTypeManager->getStorage('ap')->getQuery()
    ...
    if (!$entity->isNew()) {
      $query->condition('ap_id', $entity->id(), '<>');
    }

    $existing_aps = $query->execute();

    // If we found existing APs, add a violation.
    if (!empty($existing_aps)) {
